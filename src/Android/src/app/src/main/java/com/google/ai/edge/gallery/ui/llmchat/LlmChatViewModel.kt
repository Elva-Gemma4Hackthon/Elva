          ...
          { partialResult, done, partialThinkingResult ->
            if (partialResult.startsWith("<ctrl")) {
              ...
              if (done) {
                ...
                // Elva LaoBai: Speak the response aloud for elderly users.
                // Get the final text message content and speak it.
                val finalTextMessage = getLastMessage(model = model)
                if (finalTextMessage is ChatMessageText) {
                  com.elva.laobai.ElvaTtsManager.speak(finalTextMessage.content)
                }
              }
            }
          }
